...
使用数组存储对象存在一些不足之处，因为在实际开发中，存储的数据类型多种多样且数量不确定
...
**第一次添加的时候,mincapacity = 10 , 然后会进入grow方法,里面oldcapacity=0,扩容之后也是0,然后触发max(10,0)=10**
...
java集合中, 使用增强for循环遍历元素，并尝试删除其中的元素或者添加集合中的元素,都会抛出ConcurrentModificationException

原因 : 

增强for本质是iterator , 在iterator.next()方法中 , 会判断 modcount 和exceptedModCount是否相等,不相等就抛出异常
...
![](Attachments/Images/Pasted%20image%2020240315165053.png)
...
- hash值如果足够随机,在附在因子0.75的情况下,长度超过8的链表概率很小,是0.00000006,选择8就是让树化几率足够小
...
## 为什么使用asList转化出来的集合调用add,remove,clear会抛异常

`Arrays.asList()` 方法返回的并不是 `java.util.ArrayList` ，而是 `java.util.Arrays` 的一个内部类,这个内部类并没有实现集合的修改方法或者说并没有重写这些方法

补充 : `Arrays.asList()`是泛型方法，传递的数组必须是对象数组，而不是基本类型
...
3. 调用 `peek` 方法看看当前队首元素是否就是本次入队的元素,如果是则说明当前这个元素是即将到期的任务(即优先级最高的元素)，于是将 `leader` 设置为空,通知因为队列为空时调用 `take` 等方法导致阻塞的线程来争抢元素。因为之前leader如果不为null那么说明有线程已经进入了有限等待状态 , 但是由于我们已经更新了最小元素 , 所以需要释放leader让他们重新竞争
4. 上述步骤执行完成，释放 `lock`。
5. 返回 true。
...
![](Attachments/Images/Pasted%20image%2020240330003144.png)
...
![](Attachments/Images/Pasted%20image%2020240330003214.png)
