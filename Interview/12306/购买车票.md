...
1. 验证购票的车次是否存在
2. 查询是否是在购票时间段内
3. 获取这趟列车所有的站点 , 查询车站是否存在车次中并且车站的顺序是否正确
...
2. 遍历这个map,然后从缓存中获取到对应的票,如果没有那就查询数据库,查询车票文章已经细讲过 , 简单来讲就是查询seat表count一下状态为available的座位数量
3. 判断余票是否充足
...
1. 获取到令牌容器,结构为hash , **{prefix+trainId:{起始站点+结束站点+座位类型,票数}}**,如果不存在那么就加载 , 也是双重判定锁 , 查询seat表获取到座位类型以及对应数量
2. 准备lua脚本执行数据,先获取到执行的lua脚本,然后统计出每个座位类型需要的数量,最后还要得出需要扣减的站点 , 然后还需要根据座位类型分类得到每个类型的数量
...
5. 策略模式抽象类中会调用对应的实现类选择座位 , 然后返回选择的座位的信息
...
## order-service创建订单 , 发送延时消息

1. 生成一个订单号
2. 构建订单参数,将订单插入订单表中,再构建每一个order_item,批处理插入 , 都是待支付状态
...
将订单的状态改成超时取消close状态 , 子订单也是close状态

### canal监听到order表变化 , 更新余票缓存

1. 根据订单号查询到订单的详情 , 里面包含了子订单详细信息
2. 遍历子订单 , 然后根据里面的开始站点结束站点解锁票 , canal监听到seat表的状态 , 会自动增加车票缓存
